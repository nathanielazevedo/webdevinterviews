...
import { RoomsService, UsersService } from '@webdevinterviews/shared';
import type {
...
export const useRoomBattle = (roomId: string) => {
  const [battle, setBattle] = useState<BattleStatusResponse | null>(null);
  ...
  const fetchBattle = useCallback(async () => {
    ...
    try {
      const response = await RoomsService.getBattle(roomId);
      setBattle(response);
    } catch (err) {
...
export const useRoomPlayers = (roomId: string) => {
  ...
  const fetchPlayers = useCallback(async () => {
    ...
    try {
      const response = await RoomsService.getPlayers(roomId);
      setPlayers(response);
    } catch (err) {
...
export const useUserBattles = (userId: string, limit = 50) => {
  const [battles, setBattles] = useState<BattleHistoryResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchBattles = useCallback(async () => {
    ...
    try {
      const response = await UsersService.getBattles(userId, limit);
      setBattles(response);
    } catch (err) {
...
export const useRoomsStatus = (roomIds: string[]) => {
  const [roomsStatus, setRoomsStatus] = useState<MultiRoomStatusResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const fetchRoomsStatus = useCallback(async () => {
    ...
    try {
      const response = await RoomsService.getStatuses(roomIds);
